...
local datas = {
  ...
  tween_anims = {
    ...
    hide = tws:Create(board, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
        Position = UDim2.new(1.2, 0, 0.18, 0)
    })
  }
}

repeat wait() until type(datas.buttons.idk) == "table"

for index = 2, #datas.folders do
  if not core:FindFirstChild(datas.folders[1]) then
    Instance.new("Folder", core).Name = datas.folders[1]
  end if not core[datas.folders[1]]:FindFirstChild(datas.folders[index]) then
...
dashboard.MouseButton1Click:Connect(function()
    if not datas.buttons.vars.toggle_ui then
        ...
        dashboard.TextColor3 = Color3.new(1, 1, 1)
    end datas.buttons.vars.toggle_ui = not datas.buttons.vars.toggle_ui
end)
